...
This seemed to boost the R-squared value quite a lot. However, now there are a lot of variables in the output, and I'm worried that it may eventually overfit the data. To help cut down on this, I noticed that a few neighborhoods yielded very low p-values, and others not so much. Therefore, I recoded the Neighborhood variable accordingly:  
...
And finally, the residuals when plotted on a q-q plot appear to be pretty normally distributed, although a bit skewed towards the maximum and minimum. We can see from the histogram that the distribution is unimodal and is slightly left skewed, however, it is approaching normal so I feel that this qualifies as a valid multiple regression model.  
